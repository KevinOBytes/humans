...
preserving the public GraphQL citation shape. Protected links require the
configured deployment data key, store no plaintext locator or quote, and decrypt
only after workspace, person, identifier visibility, and current-version checks
pass. Unauthorized readers omit protected candidates; malformed storage and
out-of-band sensitivity reclassification fail closed; active-citation guards
cover legacy path prefixes, and audit events remain field-metadata only. A
focused unit suite covers purpose/key binding and storage-mode guards. The live
PostgreSQL integration remains gated when `TEST_DATABASE_URL` is absent, so this
bounded evidence does not close hosted or whole-profile acceptance.
...
GraphQL, and the protected jobs boundary. Hosted credentialed sign-in/person
...
Governed executor integrity checkpoint (2026-09-14): the deletion worker now
locks the governing privacy request and requires its approved person/file scope
to match the queued scope before mutation. Invalid, substituted, unavailable,
and unsupported scopes terminalize both request ledgers with one redacted audit
in the same transaction; replay does not repeat effects or audit events. Focused
PostgreSQL coverage includes concurrent rejection, audit-failure rollback,
cross-workspace fencing, person/file irreversible-action rejection, legal holds,
and processor retry without repeating local deletion. Hard-delete and
anonymization remain unavailable: complete processor coverage and a
...
25 older rows from rejected requests proves that a valid later request advances
while rejected request, processor, and audit records remain unchanged.
...
Accepted-history checkpoint (2026-09-13): reviewed AI suggestions are discoverable at the person research panel through a generated, permission-gated GraphQL connection scoped to the active workspace, person, governed purpose, and optional case. Pagination is bounded and deterministic. The projection contains immutable acceptance metadata, provider/model and originating run, reviewer and decision timestamps/reason, confidence/uncertainty, field, accepted resource, and accepted evidence references. Resource and workspace-evidence identifiers/details are rechecked against current permissions and visibility; inaccessible evidence is explicitly redacted instead of leaking its identifier, locator, or quote. Pending/deferred rows remain available only through the separate review queue. Focused live PostgreSQL service and generated-GraphQL tests plus component tests cover accepted-versus-pending separation, exact-purpose filtering, cross-workspace denial, pagination, evidence redaction, and explicit empty/redacted UI states. Hosted-provider and whole-product accessibility evidence remain open, so HUM-FR-023 remains incomplete.
...
Bounded HUM-NFR-008 core-person evidence (2026-09-13): generated `CreatePerson`, `UpdatePerson`, and `ArchivePerson` now use the durable principal-bound HMAC response-reference ledger while preserving optional-key legacy callers. Disposable PostgreSQL acceptance, included in the required database CI command, proves concurrent lifecycle replay, one principal claim and redacted audit per mutation, changed-material conflict rejection, malformed-reference failure, serialized expiry takeover, and workspace/principal raw-key fencing. The stored references are versioned opaque person identities and every replay rechecks current workspace visibility and optimistic version. Remaining people and retryable job/settings mutation families remain open.

Bounded HUM-NFR-008 person-file attachment evidence (2026-09-13): generated `AttachPersonFile` and `ArchivePersonFile` now consult the principal-bound durable ledger before current-row shortcuts and replay only an exact opaque attachment/version reference. Disposable PostgreSQL acceptance in `graphql-person-file-idempotency.test.ts`, included in the required database CI command, proves concurrent attach/archive convergence, one current attachment and redacted audit effect per operation, changed-material and stale-version conflicts, malformed create/archive reference rejection before attachment lookup, serialized expiry takeover after archival, and raw-key workspace/principal fencing. Replays recheck current person/file visibility, archived detach responses remain replayable at their committed version, and optional unkeyed callers remain unchanged. HUM-NFR-008 and the remaining people/job/settings mutation families remain incomplete.

Bounded core-person upgrade-compatibility evidence (2026-09-13): the principal runner now checks only the matching, unexpired legacy user claim for `person.create`, `person.update`, or `person.archive` before creating a new principal claim. Disposable PostgreSQL acceptance and independent review prove base-to-current replay for all three without duplicate domain/audit effects, including the historical versionless `{ personId }` create reference. Legacy create replay preserves the old active-row boundary and returns `NOT_FOUND` after archival; update/archive remain version-fenced. This is a migration bridge, not completion of the remaining retryable-mutation matrix.
...
Bounded HUM-FR-023 person-hold retention evidence (2026-09-13): AI-thread retention candidate selection and its lock-protected destructive recheck now preserve every private expired thread with a completed run that explicitly scoped a person under an active workspace-scoped legal hold. The recheck uses the same workspace advisory lock as hold creation/release, so a queued hold is committed before purge can proceed; direct thread/run/suggestion/citation/ephemeral-input hold fencing remains intact. Live PostgreSQL tests prove held threads produce no purge audit, independent release permits one redacted purge, direct child holds still preserve a thread, and the queued hold-versus-purge race preserves the thread. This is a bounded AI retention relation; provider, web-research snapshot, and complete retention-policy matrices remain open.
...
Bounded HUM-FR-029 temporal-editor evidence (2026-09-13): the authorized React Flow neighborhood editor now exposes relationship temporal semantics, precision, and valid-from/valid-until fields for create and update review. Create drafts retain the temporal values without writing; only the explicit confirmation sends them through the existing generated GraphQL mutation. Updating an existing edge retains its exact stored temporal instants until a reviewer edits them; the `datetime-local` controls render and parse in the browser's local timezone, and intentional edits retain the entered seconds and milliseconds through ISO normalization. A confirmed edit can explicitly clear date bounds or reset unknown temporal metadata. `tests/unit/graph-accessibility.test.tsx` covers no-write-before-confirmation plus temporal create/update, exact-instant preservation, local-time editing, precision preservation, and clear/reset payloads. `tests/e2e/graph.spec.ts` was authored to capture the authenticated browser CreateRelationship request with temporal fields, but the local invocation could not run because `TEST_DATABASE_URL` is not configured; database-backed CI or hosted execution remains required. The editor remains capped to the authorized neighborhood and still delegates workspace, consent, authorization, optimistic-version, audit, and idempotency enforcement to the server; full editor breadth and performance/provider matrices remain open.
...
| `HUM-FR-035` | Vercel deploys the same application behavior with managed PostgreSQL, Redis, S3-compatible storage, Resend, and configured AI endpoint; migration and bounded scheduled job execution are documented and verified.                                                                                                                                  | `tests/unit/vercel-deployment-contract.test.ts` and `tests/unit/jobs-route.test.ts` verify the standalone Next build contract, Node 24 runtime declaration, schema-only Vercel cron configuration, exact `CRON_SECRET` bearer behavior, public/secret environment separation, malformed/unsafe smoke inputs, no-URL skip behavior, and local URL smoke. Current live evidence verifies `humans.kevinbytes.com` and Ready production deployment `dpl_3RCBQcxsrNdAnfaLgfFXtwjbr9Nk` built from runtime commit `c3139ab`, with the configured managed-provider environment topology retained. Fresh public smoke verifies `/api/health/live`, `/api/health/ready` with PostgreSQL/Redis/storage, unauthenticated `/api/graphql` and `/api/jobs/run` stable `401` envelopes, and the production build. Direct probes against the alias returned DELETE jobs `405`, OPTIONS storage `204`, and PATCH GraphQL `405`, each with a correlation ID and `private, no-store`. Hosted bootstrap repeat/recovery, authenticated sign-in/create-person acceptance, and the full hosted provider matrix remain release work. Ordinary request-time sign-in and jobs bootstrap are non-rotating; password rotation is available only through the attended operator command, and protected Vercel secrets were not exported.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Incomplete |
...
### Bounded production-loader recovery evidence

`HUM-FR-033` remains incomplete. The production GraphQL and Better Auth loaders
now evict rejected application-initialization promises; the storage loader uses
its existing outer retry cache without retaining a second rejected promise.
`tests/unit/production-loader-recovery.test.ts` exercises the actual exported
routes with a synthetic transient initialization failure: concurrent requests
share the failed attempt, later requests recover while retaining unauthenticated
or unsigned-request denials, and successful initialization remains cached.
Failure responses retain route-specific codes, sanitized request IDs, redacted
logs and bodies, and `private, no-store`. Requests and mutations are not replayed.
This proves application-level retry behavior only: native ESM evaluation failures
may remain cached by the runtime and require process restart or redeployment.
Hosted/provider outages and the whole-application failure matrix remain open.

### Bounded assignment-queue browser evidence

On 2026-09-13, the disposable authenticated Chromium fixture passed one
fictional owner/reviewer/viewer journey against PostgreSQL. It selected an
authorized seeded case; rendered and filtered the bounded 25-row queue;
created, assigned, transitioned, and keyboard-escalated a queue row with
required reasons; and verified the resulting generated GraphQL response,
database item/event state, and unchanged case-resource links. The same journey
checks a foreign viewer receives `NOT_FOUND` for the case-scoped generated
GraphQL list and does not receive the case in the browser selector. It also
checks the native labeled filter control, mobile no-horizontal-overflow, and an
axe scan of the queue region. This test is included in the required
...
Bounded direct-route acceptance evidence (2026-09-13): the dynamic
`tests/unit/nfr006-security-contract.test.ts` contract now proves every
discovered API route preserves a valid correlation ID and replaces malformed
values at the shared proxy response boundary. The proxy emits that
validated/generated `x-request-id` with its existing browser-security headers.
Focused direct account-route coverage proves unauthenticated browser denials
and invitation-handoff cookie non-disclosure; GraphQL security coverage adds
malformed API-key redaction, correlated `private, no-store` denials, and
foreign-workspace header non-disclosure. Live GraphQL cases still require
`TEST_DATABASE_URL`, so this is local bounded evidence rather than whole-MVP
...
| `HUM-NFR-012` | Dedicated tests cover cross-workspace attacks, admin bootstrap, username/email auth, invitations, roles, TOTP, backup codes, API keys, GraphQL limits, CSRF/origin, upload validation, log redaction, and authorization bypass. The primary end-to-end journey uses a deterministic fake AI provider.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Task 12 browser coverage exercises tenant isolation, safe snippets, saved searches, protected-value leakage, snapshot validity/drift, deterministic analysis, export, accessibility, and workspace state reset. The Chromium research journey now also proves deterministic AI auto-fill, consent gating, selective field application, and tenant-scoped persistence. The auth catch-all now denies all Better Auth global admin routes before adapter initialization; `tests/unit/auth-route-boundary.test.ts` covers GET/POST/PATCH/PUT/DELETE and verifies that no Better Auth loader or endpoint is reached. Authentication/provider and the complete security journey remain open.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Incomplete |
...
| `HUM-NFR-018` | MVP completion requires the full current requirement matrix, migrations, generated artifacts, tests, builds, local PostgreSQL/Redis/MinIO Compose stack, and relevant runtime checks to pass; any remaining TODO is explicitly post-MVP.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | GitHub Actions run `34737257196` for runtime commit `c3139ab` passed all 9 jobs, including Node 24 PostgreSQL integration, browser acceptance (38 passed, 3 skipped), production build, Compose lifecycle, generated drift, quality, image security, dependency policy, and secret scanning. Documentation commit `462131b` passed all 9 jobs in run `34737675903`; Vercel deployment `dpl_3RCBQcxsrNdAnfaLgfFXtwjbr9Nk` is Ready and serves `humans.kevinbytes.com`, with public smoke and direct-route probes passing. This remains repository/CI/deployment-smoke evidence only and does not substitute for hosted authenticated/provider/runtime proof or the remaining incomplete TODO rows.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Incomplete |
...
| `HUM-NFR-020` | Production-hardening performance budgets use a representative 10,000-person/25,000-edge workspace: authenticated GraphQL reads are p95 at or below 500 ms and mutations at or below 750 ms under 20 concurrent users; graph initial render is at or below 3 s and pan/zoom sustains at least 30 FPS on the documented reference desktop; public routes meet p75 LCP at or below 2.5 s, INP at or below 200 ms, and CLS at or below 0.1; initial route JavaScript is at or below 250 KiB compressed.                                                                                                                                                                                                                                                                            | Task 12 includes bounded statement timeouts, algorithm caps, and representative PostgreSQL GIN-plan evidence. The initial disposable run measured graph-read p95 at 699.10 ms because the visible-person sort lacked a matching index. Migration `0044_core.sql` adds the workspace-leading partial expression index used by that exact sort; a subsequent isolated run seeded the exact 10,000-person/25,000-edge fixture, measured authenticated concurrent graph-read p95 at 129.96 ms, and passed all three graph performance tests. This is local evidence, not reference-desktop or hosted proof. Repeatable mutation/upload latency, graph render/FPS, Web Vitals, route-budget, hosted performance, and reference-machine evidence remain required.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Incomplete |

### NFR-008 API-key create/rotate bounded evidence

The API-key create and rotate mutations now accept optional principal-bound
idempotency keys. Their response references contain only opaque action,
outcome, and request material; the one-time credential remains transient to
the first successful executor and a replay returns `replayed: true` without a
recoverable secret. Keyed rotation commits the replacement, replacement
principal, original disable, and redacted audit atomically. Focused live
PostgreSQL coverage proves concurrent convergence, one transient secret, no
plaintext in the stored key or response ledger, changed-material conflicts,
and workspace-principal fencing. Malformed-reference/expiry-takeover coverage,
lost-first-response recovery, legacy unkeyed callers, and the whole retryable
mutation matrix remain open.

The graph-read performance gap now has a bounded local remediation. The initial
run measured 699.10 ms p95 because the visible-person sort lacked a matching
index; migration `0044_core.sql` adds the workspace-leading partial expression
index and the subsequent isolated run measured 129.96 ms p95 with all three
graph performance tests passing. `Repeatable latency, concurrency, Web Vitals`
remain required, along with graph-frame-rate, route-budget, mutation/upload,
hosted-performance, and reference-machine proof for `HUM-NFR-020`.
...
- Create and rotate now accept optional principal-bound durable idempotency
  keys. Their response references contain only opaque action/result/request
  material; the one-time credential remains request-local to the first
  successful executor and a replay explicitly returns `replayed: true` without
  a recoverable secret. Keyed rotation atomically commits the replacement,
  replacement principal, original disable, and redacted audit. Focused live
  PostgreSQL acceptance proves concurrent create/rotate convergence, one
  transient secret, no plaintext in the stored key or response ledger,
  changed-material conflicts, workspace-principal fencing, and the prior
  revoke controls. Malformed-reference and expiry-takeover coverage for
  create/rotate, recovery after a lost first response, legacy unkeyed callers,
