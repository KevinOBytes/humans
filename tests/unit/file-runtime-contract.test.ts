...
describe("file upload runtime wiring", () => {
  it("threads deployment mode into the production file runtime and server UI limits", () => {
    ...
    expect(storageRoute).toContain("createUploadSessionProxyExecutor");
    expect(storageRoute).toContain("database: db");
    expect(storageRoute).toContain("deploymentMode: env.DEPLOYMENT_MODE");
