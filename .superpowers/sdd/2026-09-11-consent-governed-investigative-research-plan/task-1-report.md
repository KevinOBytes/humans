...
- Corrected the pure scope evaluator so field and case constraints are matched conjunctively by one consent-scope row. A request omitting either dimension now matches only an explicit wildcard, preventing scoped rows from being broadened by omission or mixed across records.
- Added regression coverage for omitted field/case dimensions and cross-row combinations.
- Coverage now compares a field policy sensitivity ceiling against the field definition sensitivity before allowing a field-scoped decision.
- Added governance tables to the exhaustive workspace/versioned schema-contract inventory and all four tables to the full-schema required-table inventory.
- Follow-up focused verification passed: `pnpm vitest run tests/unit/governance-coverage.test.ts tests/unit/domain-conventions-contract.test.ts tests/integration/full-schema.test.ts` (3 files, 112 tests), Prettier on the touched governance/schema/test files, and `pnpm typecheck`.
