...
- [ ] `HUM-NFR-008` Extend Task 18's durable response-reference replay, expiry takeover, malformed-reference rejection, and concurrent current-primary coverage across every remaining retryable mutation domain. Import `startImport` and the fact-create service transaction seam now have live PostgreSQL replay/concurrency evidence, including malformed and expired opaque references plus tenant fencing. Generated GraphQL `createUploadSession` and `sendWebhookTestEvent` now add the same bounded evidence: concurrent callers converge on one resource, malformed references fail closed before UUID lookup, expired claims take over, and raw-key reuse is tenant-isolated. Direct GraphQL idempotency for people/facts, complete/upload evidence mutations, and the remaining job/settings matrix are still open.
