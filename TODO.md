...
`dpl_8RUvPuG3DeMJ9snQXKxzZK71RFrD`; the custom hostname,
...
- [ ] `HUM-NFR-018` Produce current full-matrix MVP release evidence. The current tree now has a passing Node 24 production build, 210 test files/1,900 tests passed (5 skipped), lint, typecheck, formatting, generated drift, focused graph/performance checks, and an isolated PostgreSQL/Redis/MinIO Compose smoke with administrator recovery; the full current matrix, hosted deployment/provider/runtime proof, and all remaining TODO rows are still outstanding.
