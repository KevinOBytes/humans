...
- Bounded HUM-FR-004 provider-failure evidence (2026-09-13): generated `IssueWorkspaceInvitation` acceptance against real PostgreSQL with a failing email-provider seam returns `APPLIED` without rolling back the invitation transaction. The pending invitation remains paired with one AES-GCM-sealed outbox intent in `queued` state, with only the stable `delivery_unavailable` retry code and redacted audit effect; a later worker retry completes with the original provider idempotency key. This proves local failure recovery only; hosted Resend delivery, outage telemetry, cancellation-wins browser coverage, and the exhaustive role/recipient matrix remain open.
