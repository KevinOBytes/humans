...
- [ ] `HUM-NFR-008` Extend Task 18's durable response-reference replay, expiry takeover, malformed-reference rejection, and concurrent current-primary coverage across every remaining retryable mutation domain. Import `startImport` and the fact-create service transaction seam now have live PostgreSQL replay/concurrency evidence, including malformed and expired opaque references plus tenant fencing. Generated GraphQL `CreateFact` and `CreateEvidenceItem` now add the same bounded evidence: concurrent callers converge on one fact/evidence row, malformed references fail closed before UUID lookup, expired claims take over, and raw-key reuse is tenant-isolated. Generated GraphQL `createUploadSession`, `sendWebhookTestEvent`, and `UpdateAccessPolicy` also retain their bounded evidence. Evidence creation preserves source/file authorization, checksum validation, audit/search writes, and legacy callers. Direct GraphQL idempotency for people, complete/upload evidence mutation, and the remaining job/settings matrix are still open.
...
- Bounded HUM-NFR-008 evidence (evidence create): `createEvidenceItem` now supports durable HMAC response-reference replay; live generated GraphQL/PostgreSQL coverage proves concurrent convergence, malformed-reference rejection before UUID lookup, expiry takeover, and workspace fencing while preserving source/file authorization, checksum validation, audit/search writes, and legacy callers. Complete/upload evidence and the remaining job/settings matrix remain open.
