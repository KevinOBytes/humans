...
- [ ] `HUM-NFR-008` Extend Task 18's durable response-reference replay, expiry takeover, malformed-reference rejection, and concurrent current-primary coverage across every remaining retryable mutation domain. Import `startImport` and the fact-create service transaction seam now have live PostgreSQL replay/concurrency evidence, including malformed and expired opaque references plus tenant fencing. Generated GraphQL `CreateFact` now adds the same bounded evidence: concurrent callers converge on one fact, malformed references fail closed before UUID lookup, expired claims take over, and the raw key is tenant-isolated. Generated GraphQL `createUploadSession`, `sendWebhookTestEvent`, and `UpdateAccessPolicy` also retain their bounded evidence. Direct GraphQL idempotency for people, complete/upload evidence mutations, and the remaining job/settings matrix are still open.
